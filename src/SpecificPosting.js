...
class SpecificPosting extends Component {
  constructor(props) {
    ...
    this.setState({ postingID: vars.postingID });
  }

  componentDidMount() {
    Axios.get(
      "https://whispering-fortress-23669.herokuapp.com/specificPosting",
      {
        params: { postingId: this.state.postingID }
      }
    ).then(
      response => {
        this.setState({ post: response.data.postingDetails });
      },
      error => {
        console.log(error);
      }
